...
lớn gần đây chứng minh là hiệu quả. Mô hình có khoảng $250$ triệu tham số, giữ kích thước
...
\begin{figure}[ht]
  ...
  \begin{tikzpicture}[
    node distance=4.5mm,
    box/.style={draw, rounded corners, minimum width=40mm, minimum height=7.5mm, align=center, font=\small},
    add/.style={draw, circle, inner sep=1.5pt, font=\small},
    lbl/.style={font=\scriptsize\itshape, text=gray!90},
    >={Latex[length=2mm]}
  ]
    \node (in) {\small Chuỗi token};
    \node[box, fill=orange!12, above=of in]  (emb) {Nhúng token};
    \node[box, fill=blue!8,   above=7mm of emb] (n1)  {RMSNorm};
    \node[box, fill=orange!15, above=of n1]  (att) {Tự chú ý đa đầu + RoPE};
    \node[add, above=of att]                 (a1)  {$+$};
    \node[box, fill=blue!8,   above=of a1]   (n2)  {RMSNorm};
    \node[box, fill=green!12, above=of n2]   (ffn) {FFN SwiGLU};
    \node[add, above=of ffn]                 (a2)  {$+$};
    \node[box, fill=blue!8,   above=7mm of a2] (fn) {RMSNorm};
    \node[box, fill=gray!10,  above=of fn]   (head) {Đầu MLM (tách rời)};
    \node[above=4mm of head] (out) {\small Phân phối trên từ vựng};
    % luồng chính
    \draw[->] (in)--(emb);   \draw[->] (emb)--(n1);
    \draw[->] (n1)--(att);   \draw[->] (att)--(a1);
    \draw[->] (a1)--(n2);    \draw[->] (n2)--(ffn); \draw[->] (ffn)--(a2);
    \draw[->] (a2)--(fn);    \draw[->] (fn)--(head); \draw[->] (head)--(out);
    % kết nối tắt
    \draw[->] ($(n1.south)+(0,-2mm)$) -- ++(-2.9,0) |- (a1.west);
    \draw[->] ($(a1.north)+(0,1.5mm)$) -- ++(-2.9,0) |- (a2.west);
    % khung x28
    \node[draw, rounded corners, dashed, inner sep=3mm, fit=(n1)(att)(a1)(n2)(ffn)(a2),
          label={[font=\small]right:$\times\,28$}] {};
  \end{tikzpicture}
  \caption{Kiến trúc NeoBERT.}
  \label{fig:neobert-arch}
\end{figure}

Điểm cần chú ý khi đọc Hình~\ref{fig:neobert-arch}: khác với sơ đồ BERT quen thuộc,
...
truyền thống yêu cầu cam kết trước tổng số bước huấn luyện -- điều khó đáp ứng khi CPT diễn
ra theo nhiều phiên nối tiếp trên hạ tầng tính toán giới hạn. Lịch
