...
tỉa và đồng bộ từ vựng (Mục~\ref{sec:vocab}), quá trình trích xuất tập neo -- được mở rộng
bằng các cặp dịch, một cải tiến của đề tài so với SALT gốc (Mục~\ref{sec:anchor}), và phép chiếu SALT áp dụng
...
Một lựa chọn thay thế là dựng ma trận đầu ra bằng \textit{một ánh xạ tuyến tính toàn cục}
(global map) học chung cho mọi token thay vì ánh xạ cục bộ theo từng token. Chúng tôi chọn
phương án chiếu theo từng token cho cả hai ma trận vì lý do hình học đã nêu ở
Mục~\ref{sec:geometry}; hiệu quả của lựa chọn này so với ánh xạ toàn cục được đánh giá bằng
thực nghiệm trong Chương~\ref{chap:experiment}.
% TODO(hình so sánh): chèn hình so sánh các phương án khởi tạo đầu ra
% (per-token SALT vs global map) và tác dụng của giai đoạn căn chỉnh đóng băng.
% Người dùng sẽ cung cấp ảnh report; đặt vào Chương 4 (Kết quả) kèm phân tích.
...
% TODO(hình so sánh): chèn hình/bảng so sánh có -- không có giai đoạn căn chỉnh đóng băng
% (ví dụ đường cong MLM/độ đo hạ nguồn) vào Chương 4. Người dùng sẽ cung cấp ảnh report.
